...
def customer_login(request):
	if request.method == "POST":
		...
		user = authenticate(username = customer_name,password = customer_password)
		if user is not None:
			login(request,user)
...
def updateItem(request):
	...
	filt_product = Product.objects.filter(name = product_name)
	if filt_product is not None:
		added_by = filt_product[0].prd_addedBy
		price = filt_product[0].price
		order, created = Order.objects.get_or_create(customer=customer, complete=False)
		orderItem, created = OrderItem.objects.get_or_create(order=order, 
															product=product,
															ord_prd_addedBy = added_by,
															customer_name = customer,
															price = price)
		if action == 'add':
			orderItem.quantity = (orderItem.quantity + 1)
			orderItem.price = (price * orderItem.quantity)
		elif action == 'remove':
			orderItem.quantity = (orderItem.quantity - 1)
